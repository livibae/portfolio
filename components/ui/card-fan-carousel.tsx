...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  const hovered = hoveredIndex !== null ? cards[hoveredIndex] : null;
  ...
  return (
    <section className="relative z-20 flex w-full flex-col items-center px-4 py-2 md:px-8">
        ...
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            ...
            if (card.linkUrl.startsWith("http")) {
              return (
                <a key={index} href={card.linkUrl} target="_blank" rel="noopener noreferrer" className="fan-card block cursor-pointer">{image}</a>
              );
            }
            return (
              <Link key={index} href={card.linkUrl} className="fan-card block cursor-pointer">{image}</Link>
            );
          })}
        </div>
      </div>

      <div className="mt-2 flex h-14 max-w-lg flex-col items-center justify-start px-6 text-center">
        {hovered?.title && (
          <>
