...
const cardVariants = cva("absolute will-change-transform", {
  variants: {
    variant: {
      ...
      memo: "flex size-full flex-col items-start justify-between gap-6 rounded-sm border-none bg-[#f7e38a] p-8 text-left text-[#3d3424] md:p-10",
    },
  },
  defaultVariants: {
    variant: "light",
...
>(
  (
    ...
    const holdsLastCard = isMemo && index >= arrayLength + 1;
    const memoSpan = 0.78;
    const start = holdsLastCard
      ? 1
      : (index / (arrayLength + 1)) * (isMemo ? memoSpan : 1);
    const end = holdsLastCard
      ? 1.01
      : ((index + 1) / (arrayLength + 1)) * (isMemo ? memoSpan : 1);
    const range = React.useMemo(() => [start, end], [start, end]);
    const rotateRange = [range[0] - 1.5, range[1] / 1.5];
    const y = useTransform(scrollYProgress, range, ["0%", isMemo ? "-120%" : "-180%"]);
    const x = useTransform(scrollYProgress, range, ["0%", isMemo ? "220%" : "0%"]);
    const rotate = useTransform(
      scrollYProgress,
      isMemo ? range : rotateRange,
      isMemo ? [0, 28] : [incrementRotation, 0],
    );
    const transform = useMotionTemplate`translateZ(${
      index * incrementZ
    }px) translateX(${x}) translateY(${y}) rotate(${rotate}deg)`;
    ...
    const lightFilter = useMotionTemplate`drop-shadow(${dx}px ${dy}px ${blur}px rgba(0,0,0,${alpha}))`;
    const filter = variant === "dark" ? "none" : lightFilter;

    const cardStyle = {
      top: index * incrementY,
      transform,
      transformOrigin: isMemo ? "left bottom" : "center center",
      backfaceVisibility: "hidden" as const,
      zIndex: Math.max(1, arrayLength + 4 - index) * incrementZ,
      filter,
      ...style,
    };
